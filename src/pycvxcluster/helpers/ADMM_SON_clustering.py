...
def admm_son(
    ...
    for i in range(num_iter):
        ...
        x[1:workers] = list(
            map(
                functools.partial(update_x, rho, lmbd, d),
                a[1:workers],
                x[1:workers],
                y,
                lambdaVal,
        ...
        primal_res = np.sqrt(
            np.sum(
                [(np.linalg.norm(x[i][0] - y[i - 1]) ** 2) for i in range(1, workers)]
            )
        )
        dual_res = np.linalg.norm(-rho * (y - y_old))
        eps_pri = np.sqrt(N) * abstol + reltol * max(
    ...
    possible_centers = list(
        map(
            functools.partial(merge_centers_locally, epsilon),
            xy,
            chunk_sizes,
            a,
            data_chunk_size,
...
def merge_centers_globally(possible_centers, epsilon):
    ...
    for i in range(len(possible_centers)):
        for j in range(i + 1, len(possible_centers)):
            sum += np.linalg.norm(
                np.asarray(possible_centers[i]) - np.asarray(possible_centers[j])
            )
            cnt = cnt + 1
    avg_dist = sum / cnt
    eps = avg_dist / epsilon
    ...
    for i in range(0, len(possible_centers)):
        if i not in removed_indices:
            ...
            for j in range(i + 1, len(possible_centers)):
                if (
                    np.linalg.norm(
                        np.asarray(possible_centers[i])
                        - np.asarray(possible_centers[j])
                    )
                    <= eps
                ):  # ilon:
                    removed_indices.append(j)
...
def loss_y1(lambdaVal, x, y, workers, d):
    val = cp.sum(
        [
            (cp.reshape(lambdaVal[i], (d, 1))).T
            @ cp.reshape(y[i] - x[i + 1][0], (d, 1))
...
def main():
    ...
    x, y, centers, allPossibleCenters, reqIters = admm_son(
        filename, folder, workers + 1, num_iter=numiter, lmbd=lmbda, rho=1, eps=epsilon
    )
